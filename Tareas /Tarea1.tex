...
Procedimiento Insercion(A,n)
	para i=1 hasta i<n hacer		// 1 variable i	y 1 variable temp
		temp=A[i]					// n-1 elementos del arreglo "A"
		j=i-1						// 1 variable j
		mientras((A[j]>temp)&&(j>=0)) hacer    
			A[j+1]=A[j]				// es el resultado del while por 2
 			j--						
		fin mientras
		A[j+1]=temp					// n-1 elementos del arreglo "A"
	fin para
fin Procedimiento
\end{lstlisting}
De tal forma que la función de complejidad espacial queda de la siguiente manera:
\[f_{e}(n))=3+n(n+1)+2(n+1)=n^{2}+3n+5\]
Para encontrar su cota \textit{O}, propongamos $g(n)=n^{2}$, $c=8$ y $n_{0}=1$\newline
Si n=1,
\center$f(1)=1^{2}+3(1)+5=8\leq 8=8(1^{2})=g(1)$
\justify
Si n=2,
\center$f(2)=2^{2}+3(2)+5=15\leq 32=8(2^{2})=g(2)$
\justify
Si n=3,
\center$f(3)=3^{2}+3(3)+5=23\leq 72=8(3^{2})=g(13)$
\justify
En general, se cumple que
\center$|f(n)|\leq c|g(n)|$ , $\forall n\geq n_{0}$.
\justify
De lo que podemos decir que $f_{e}(n)$ es de orden $O(n^{2})$.
\end{document}